...
public class OrderThreeFragment extends BaseLazyFragment {
        ...
        orderAdapter.setOnItemChildClickListener((adapter, view, position) -> {
            switch (view.getId()) {
                case R.id.btn_cancel:
                    ...
                    NetModel.getInstance().getDataFromNet("CANCEL_ORDER3", HttpUrls.CANCEL_ORDER, map);
                    break;
                case R.id.btn_pay:
                    showTipe(2, "正在加载中...");
                    ...
                    NetModel.getInstance().getDataFromNet("REPAY3", HttpUrls.REPAY, map);
    ...
    private void getData(int index) {
        ...
        map.put("state", type);
        map.put("index", index);
        NetModel.getInstance().getAllData("ORDER_DATA3", HttpUrls.GET_ORDER_DATA, map);
    }
